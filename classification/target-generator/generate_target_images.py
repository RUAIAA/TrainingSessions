...
parser.add_argument('--image_size',default=30,type=int,help='Length and height dimension of output images')
...
if __name__ == '__main__':
	...
	annotations = {
		"Shape": {shape_class_dirs[i]: i for i in range(len(shape_class_dirs))},
		"Alphanumeric": {alphanumeric_choices[i]: i for i in range(len(alphanumeric_choices))},
		"Background Color": {color_choices[i]: i for i in range(len(color_choices))},
		"Alphanumeric Color": {color_choices[i]: i for i in range(len(color_choices))}
	}
	json.dump(annotations, open(annotations_filepath, 'w'), indent=0)
